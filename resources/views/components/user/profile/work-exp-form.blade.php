<form class="space-y-4" action="{{ route('profile.add.work.exp.store') }}"
    onsubmit="handleSubmit(event, appendWorkExperience)">
    <!-- Job Title -->
    <div>
        <x-label>Designation</x-label>
        <x-input name="designation" placeholder="e.g. Senior Software Engineer" />
    </div>

    <!-- Company Name -->
    <div>
        <x-label>Company Name</x-label>
        <x-input name="company_name" placeholder="e.g. Acme Inc." />
    </div>

    <!-- Location -->
    <div>
        <x-label>Company Location</x-label>
        <x-input id="location" name="company_location" placeholder="e.g. New York, NY" />
    </div>
