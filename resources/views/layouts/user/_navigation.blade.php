                        ...
                        @foreach ($dropdown_links as $link)
                            <li>
                                <a href="{{ route($link['route']) }}"
                                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">{{ $link['title'] }}</a>
                            </li>
                        @endforeach
                        <li>
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="block px-4 w-full py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Sign out</button>
                            </form>
                        </li>
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-user">
            <ul
                ...
                @foreach ($links as $link)
                    <li>
                        <a href="{{ route($link['route']) }}"
                            class="block py-2 px-3 {{ request()->routeIs($link['route']) ? 'text-white bg-blue-700 rounded-sm md:bg-transparent md:text-blue-700' : 'text-gray-900 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700' }} md:p-0 md:dark:text-blue-500"
